...
public class GranblueBosses implements
    ...
    * */
    ...
    public void receivePostInitialize() {
        ...
        ModPanel settingsPanel = new ConfigMenu();
                    ...
                    if (s == null) { //If no defined value, determine path using field name
                        ...
                        for (String ext : AUDIO_EXTENSIONS) {
                            ...
                            if (Gdx.files.internal(testPath).exists()) {
                                ...
                                logger.info("loaded SFX with key: " + s);
                                f.set(null, s);
                                continue outer;
                            }
                        }
    ...
    public static String intentPath(String file) {
        return resourcesFolder + "/ui/intents/" + file;
    }
    public static String videoPath(String file) {
        return resourcesFolder + "/video/" + file;
    }
    ...
    private static String checkResourcesPath() {
        String name = GranblueBosses.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + GranblueBosses.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(GranblueBosses.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
